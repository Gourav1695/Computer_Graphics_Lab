...
// 1. Develop a class for circle using Midpoint circle drawing algorithm. Hence draw the shape given shape.
import java.applet.*;
import java.awt.*;
...
public class circle extends Applet implements ActionListener {
    ...
    public void plotPoint(double x, double y, Graphics g, Color c) {
        ...
        g.setColor(c);
            ...
            g.fillOval((int)x * scale - (int) scale  + (int)newX, (int)newY -(int) y * scale - (int) scale , (int) scale * 2, (int) scale * 2);// creating the circle to be ploted
}
